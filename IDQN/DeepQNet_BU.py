...
class DQN(nn.Module):
    ...
    def sample_best_action(self,obs,agent=2):
        ...
        n_batch = obs.shape[0]
        ak = torch.empty([n_batch, self.n_agents])
        aJ = torch.zeros([n_batch, 1], device=self.tensor_type['device'], dtype=torch.int64)
        Qk_exp = torch.zeros([n_batch, self.n_agents], device=self.tensor_type['device'], dtype=torch.int64)
        pAnotk = torch.empty([n_batch, self.n_agents, self.n_egoA])
        ...
        with torch.no_grad():  # <=== CAUSED MEMORY ERROR WITHOUT ===
            ...
            for ibatch in range(n_batch):
                ...
                Qk_exp[ibatch, iR ] = torch.sum(qegok[ibatch, iR] * pdAegok[ibatch, iR, :], dim=2, keepdim=True)

                ak[ibatch,iH] = aH
                pAnotk[ibatch, iH,:] = pdAegok[ibatch, int(not (iH)), :]
                Qk_exp[ibatch, iH ] = torch.sum(qegok[ibatch, iH] * pdAegok[ibatch, iH, :], dim=2, keepdim=True)
